...
def confirm():
    ...
    if answer:
        ...
        name = socket.gethostname()
        index = None
        f=open(r"C:\\popupresults\\windowsslave4.txt", 'w')
        f.write('0')
        f.close()
        if name in d:
...
ssh_client.connect(hostname='174.129.111.157',username='Administrator',
                   password='xZOJR*VZvuFimZ-HmBQEiUO;Na5jbm7g',port=22, timeout=3)
